...
class ActionDestroyCodeLockOnTent : ActionContinuousBase {
    ...
    override bool ActionCondition(PlayerBase player, ActionTarget target, ItemBase item) {
        ...
        if (tent) {
            ...
            if (codelock && GetDayZGame().GetCodeLockConfig() && GetDayZGame().GetCodeLockConfig().CanRaidTents()) {
    ...
    override void OnFinishProgressServer(ActionData action_data) {
        if (!GetDayZGame().GetCodeLockConfig() || !GetDayZGame().GetCodeLockConfig().CanRaidTents()) {return;}
