modded class Fence {
    ...
    override bool CanReceiveAttachment(EntityAI attachment, int slotId) {
        if (attachment.Type() == ATTACHMENT_CODE_LOCK && (!HasFullyConstructedGate() || (GetDayZGame().GetCodeLockConfig() && !GetDayZGame().GetCodeLockConfig().CanAttachToGates()))) { return false; }
