...
class ActionDestroyCodeLockOnFence : ActionContinuousBase {
    ...
    override bool ActionCondition(PlayerBase player, ActionTarget target, ItemBase item) {
        ...
        if (fence) {
            ...
            if (codelock && GetDayZGame().GetCodeLockConfig() && GetDayZGame().GetCodeLockConfig().CanRaidGates()) {
    ...
    override void OnFinishProgressServer(ActionData action_data) {
        if (!GetDayZGame().GetCodeLockConfig() || !GetDayZGame().GetCodeLockConfig().CanRaidGates()) { return; }
