class Aurora: NamWeatherEvent
{
	...
	protected ref Timer m_MidPhaseTimer = new Timer(CALL_CATEGORY_SYSTEM);
	
	void ~Aurora()
	{
		RemoveAuroraSlices();
		delete m_MidPhaseTimer;
	}
	...
	{
		SpawnAuroraSlices();
		m_MidPhaseTimer.Run(0.1, this, "MidPhaseClientTick", null, true);
	}
		
	void MidPhaseClientTick()
	{
	...
	protected void SpawnAuroraSlices()
	{
		...
		for (i = 0; i < m_AuroraInstances.Count() - 1; i++) {
			if (i < 189) {
				vector newPos;
	...
	protected void UpdateAuroraSlices()
	{
		...
		for (int i = 0; i < m_AuroraInstances.Count() - 1; i++) {
			if (i < 189) {
				vector newPos;
				newPos[0] = playerPos[0] + m_AuroraParts[i][0];
				newPos[1] = playerPos[1] + m_AuroraAltitude;
	...
	// If you dont do this, the sky catches on fire	
